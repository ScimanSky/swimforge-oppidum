...
const configSchema = z.object({
  CLUB_WORKOUTS_AI_MODEL_PRIMARY: z.string().trim().min(1).catch("qwen2.5:7b"),
  CLUB_WORKOUTS_AI_MODEL_ESCALATION: z.string().trim().min(1).catch("qwen2.5:3b"),
  ...
  GEMINI_TEXT_TIMEOUT_MS: z.coerce.number().int().min(5_000).max(120_000).catch(30_000),
  CLUB_WORKOUTS_AI_TIMEOUT_MS: z.coerce.number().int().min(5_000).max(120_000).catch(90_000),
  CLUB_WORKOUTS_AI_REQUEST_TIMEOUT_SOFT_MS: z.coerce
    ...
    .max(120_000)
    .catch(60_000),
  LOCAL_LLM_TIMEOUT_MS: z.coerce.number().int().min(5_000).max(120_000).catch(90_000),
  LOCAL_LLM_MAX_TOKENS: z.coerce.number().int().min(64).max(8_192).catch(900),
